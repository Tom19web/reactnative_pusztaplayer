...
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    ...
    case 'ADD_HISTORY': {
      ...
      const updated = [{ ...item, addedAt: Date.now() }, ...filtered].slice(0, 20);
...
/** @deprecated use specific hooks (useCore/useFavorites/useHistory) instead */
export function useAppState(): AppState {
  const core = useCore();
  const favs = useFavorites();
  const history = useHistory();
  const activeProfile = core.state.profiles.find(p => p.id === core.state.activeProfileId);
  return {
    ...core.state,
    favorites: favs,

import { XTREAM_SERVER, USER_AGENT, CACHE_LIVE, CACHE_VOD } from '../constants';
...
async function apiGet<T = any>(
  ...
  let res: Response;
  try {
    res = await fetchWithTimeout(url, { headers: { 'User-Agent': USER_AGENT } }, 15000);
  } catch (e: unknown) {
    if (e instanceof Error && e.name === 'AbortError') throw new Error('A szerver nem válaszol (időtúllépés).');
...
// Live stream formátum: 'm3u8' (HLS, alapértelmezett) vagy 'ts' (MPEG-TS)
const LIVE_FORMAT = 'ts';

export function buildLiveUrl(u: string, p: string, id: number): string {
  return `${XTREAM_SERVER}/live/${u}/${p}/${id}.${LIVE_FORMAT}`;
}

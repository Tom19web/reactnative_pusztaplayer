...
export async function flush() {
  ...
  try {
    ...
    if (serverState.profiles.length >= 2 && (data.profiles?.length ?? 0) <= 1) {
      if (__DEV__) console.warn('[wordpressSync] read-before-write véd: szerveren több profil van, mint a kliensen — skip');
      return;
    }
  } catch {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (__DEV__) console.warn('[wordpressSync] version conflict, fetching fresh profiles');

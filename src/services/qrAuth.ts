...
export async function requestQRCode(): Promise<QrRequestResult> {
  const res = await fetchWithTimeout(`${QR_API_BASE}/qr-request?_method=POST`, {}, 15000);
  if (!res.ok) throw new Error('QR kód igénylés sikertelen (HTTP ' + res.status + ')');
...
export function pollQRCode(
  ...
  function poll() {
    ...
    attempts++;
    const url = `${QR_API_BASE}/qr-poll?code=${encodeURIComponent(code)}`;
    ...
    (async () => {
      try {
        ...
        stopPolling();
        onError('Hálózati hiba: ' + (err instanceof Error ? err.message : String(err)));
      }
    })();
  }

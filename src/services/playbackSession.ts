...
const EP_URLS_MAX = 500;

function parseEpisodeId(key: string): number | null {
  const m = key.match(/^ep_(\d+)$/);
...
export async function saveEpisodeUrl(key: string, url: string, title: string): Promise<void> {
  try {
    ...
    if (__DEV__) console.warn('[playbackSession] saveEpisodeUrl failed:', e);
  }
}
...
export async function createPlaybackSession(
  ...
  try {
    ...
    if (__DEV__) console.warn('[playbackSession] fallback lookup failed:', e);
  }

  // Fallback: építsünk URL-t credential-ökből (nincs cache, de ismerjük az ID-t)
  const epId = parseEpisodeId(contentId);
  if (epId != null) {

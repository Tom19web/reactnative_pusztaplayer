...
const STARS: { top: number; left?: number; right?: number; size: number; fill: string }[] = [
  { top: 30, left: 8, size: 20, fill: '#39FF14' },
  { top: 120, right: 10, size: 17, fill: '#FF6600' },
  { top: 210, left: 20, size: 15, fill: '#FFEE00' },
  { top: 290, right: 25, size: 18, fill: '#FF0044' },
  { top: 380, left: 6, size: 21, fill: '#39FF14' },
  { top: 460, right: 14, size: 17, fill: '#FF6600' },
  { top: 540, left: 16, size: 15, fill: '#FFEE00' },
  { top: 620, right: 8, size: 20, fill: '#FF0044' },
];

export default function SidebarBg() {
  const starData = useMemo(() => STARS.map(s => {
    const x = s.left !== undefined ? s.left : CW - (s.right || 0) - s.size;
    return { x, y: s.top, pts: comicStarburstPoints(s.size, 5, 2, 2), fill: s.fill, size: s.size };
  }), []);

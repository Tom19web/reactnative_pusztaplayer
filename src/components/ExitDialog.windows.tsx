...
import { View, Text, Image, ImageBackground, StyleSheet, Animated, BackHandler, AppState, Platform } from 'react-native';
...
export default function ExitDialog({ onDismiss }: ExitDialogProps) {
  ...
  const handleExit = () => {
    ...
    Animated.sequence([
      ...
      // Windows: BackHandler.exitApp is Android-only
      // Use native module to close window or fallback
      try {
        BackHandler.exitApp();
      } catch {
        onDismiss();
          ...
          <View style={styles.buttons}>
            ...
            <ShadowWrapper offset={6} borderRadius={12}>
              <TFPressable
                ...
                onPress={onDismiss}
                testID="cancel-btn"
                accessibilityLabel="Vissza a kezdÄąâ€kÄ‚Â©pernyÄąâ€re"
                accessibilityRole="button"
              >
                <Text style={styles.btnText}>MÄ‚Â©gse</Text>
              </TFPressable>
            </ShadowWrapper>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  goodbyeLogo: { width: 100, height: 100, borderRadius: 20 },
  goodbyeTitles: { alignItems: 'flex-start' },
  goodbyeTitle: {
    color: COLORS.yellow,
    fontSize: 36,
    fontFamily: FONT_FAMILY_BANGERS,

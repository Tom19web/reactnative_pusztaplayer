...
void WindowsVideoPlayer::CreatePlayerWindow() {
  ...
  SetLayeredWindowAttributes(m_playerHwnd, 0, 255, LWA_ALPHA);
...
void WindowsVideoPlayer::play(const std::wstring &source) {
  ...
  libvlc_event_attach(em, libvlc_MediaPlayerEndReached,
                      OnMediaPlayerEvent, this);
  libvlc_event_attach(em, libvlc_MediaPlayerTimeChanged,
                      OnMediaPlayerEvent, this);
  libvlc_event_attach(em, libvlc_MediaPlayerLengthChanged,
                      OnMediaPlayerEvent, this);
  ...
  // Set User-Agent for Xtream streams
  libvlc_media_add_option(m_media,
                          ":http-user-agent=PusztaPlayer/0.7.0");
  // Ensure HLS works
